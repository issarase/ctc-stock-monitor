...
export async function POST(request: Request) {
  try {
    ...
    // 1. ดึง CTC Credentials จาก Supabase
    ...
    // 🎯 3. ยิง per_page=52 พร้อมกัน 2 หน้าแบบขนาน (Page 1 และ Page 2)
    const pagesToFetch = [1, 2];
    const fetchPromises = pagesToFetch.map(p => {
      const targetUrl = `https://tsd.ctc.co.th/ctc_stock_prd/inv_product_list.php?view=grid&per_page=52&stock=in&q=${encodeURIComponent(querySearch)}&page=${p}`;
      return client.get(targetUrl).catch(() => null);
    });

    const pageResponses = await Promise.all(fetchPromises);

    // 4. แกะข้อมูลจากทุกหน้ารวมกัน
        ...
        if (text.includes('Stock Code:') && text.length < 1200) {
          ...
          if (stockCode && !scrapedItems.some(item => item.stockCode === stockCode)) {
            ...
            scrapedItems.push({
              name: realTitle,
              stockCode,
              oppNumber: oppMatch ? oppMatch[1].trim() : (oppNumber || 'N/A'),
    ...
    // 💾 5. บันทึกลง Supabase (user_customers & opp_items)
    ...
    if (!existingCust) {
      await supabase.from('user_customers').insert({ username, customer_name: customerName });
    }
    ...
    return NextResponse.json({ success: false, error: error.message || 'เกิดข้อผิดพลาดในการ Sync' }, { status: 500 });
  }
}